...
# binary (usr/bin/cc-switch; the frontend assets are embedded in the binary).
# We keep only that binary at a stable path the wrapper expects:
# /app/extra/cc-switch. The desktop file, icon and AppStream metainfo are shipped
...
[ -f cc-switch.deb ] || { echo "missing extra-data: cc-switch.deb" >&2; exit 1; }
